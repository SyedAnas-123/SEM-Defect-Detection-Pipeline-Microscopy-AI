...
cells = [
    nbf.v4.new_markdown_cell("# Phase 16: YOLO11m + P2 Head (FULL RUN)\n\nThis notebook dynamically creates the corrected `yolo11-p2.yaml` architecture, loads `yolo11m.pt` weights, and runs the full 200-epoch training. The objective is to significantly boost recall on sub-0.5% area defects."),
...
"""),
    ...
    nbf.v4.new_markdown_cell("## 2. FULL Training Run (200 Epochs)"),

    nbf.v4.new_code_cell("""\
import time

print("Starting FULL Run (Batch=16, 640x640, Epochs=200)...")
t0 = time.time()
...
print("Best weights saved to: runs/detect/EXP16-YOLO11m-P2-FULL/weights/best.pt")
""")
]

nb['cells'] = cells
notebook_path = os.path.join(os.path.dirname(__file__), 'notebooks', '20_exp16_yolo11m_p2_full_run.ipynb')
